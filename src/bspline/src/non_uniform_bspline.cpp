...
namespace fast_planner
{
                                              ...
                                              const double &interval)
    {
        ...
        u_ = Eigen::VectorXd::Zero(m_ + 1); // 时间节点数
        for (int i = 0; i <= m_; ++i)
        {
    ...
    double NonUniformBspline::checkRatio()
    {
        ...
        if (ratio > 2.0)
        {
            RCLCPP_ERROR(rclcpp::get_logger("non_uniform_bspline"), "max vel: %lf, max acc: %lf.", max_vel, max_acc);
        }
        return ratio;
    }
    ...
    void NonUniformBspline::parameterizeToBspline(const double &ts, const vector<Eigen::Vector2d> &point_set,
