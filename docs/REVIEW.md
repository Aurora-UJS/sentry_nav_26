...
> **解决进度（更新于本轮开发，以下分析正文保留原样）**：#1（多线程隔离控制环）、#2（点云 raycast，原已实现）、#3（replan 文档一致）、#4（footprint 对角线安全）、#6（MPC 常量矩阵缓存）、#7（速度相关裕度，默认关）均已修复（见 `wip/tracker-yaw-and-review` + PR #1）；另新增脱困恢复 FSM。**L1 真因更正**：仿真运动 Z 漂移主因是 `small_point_lio` ESKF 加速度协方差饱和索引 bug（submodule `cf4d4df` 已修，~0.19m → ~0.02m），**非本报告"缺重力"假设**。L2（仿真雷达为快照、deskew 无关）暂缓。#5 拆为两面看：**多层占用 / 净空（狗洞）判定仍暂缓**；但其中**单向 / 方向性可通行**这一 facet（高位雷达扫不到台阶立面 → 占用图把"只能下不能上"的台阶误判为双向平地）已由**静态可通行性标注层**在规划层解决——离线 `*.trav.yaml` 标注 free/obstacle/oneway，全局 JPS 软方向代价 + obstacle 叠加、局部 A*+MINCO 硬方向约束，OPT-IN 默认关（`regions: []` 即 disabled，零行为变化）。详见 `docs/TRAVERSABILITY.md`。
...
| 5 | 单层 2D 占用，狗洞/净空判定缺失 | 🟠 高 | 算法/钻洞 | 🟡 单向/方向性 facet 已由标注层解决（见 TRAVERSABILITY.md）；多层净空仍暂缓 |
