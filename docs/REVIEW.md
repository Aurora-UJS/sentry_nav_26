...
> **解决进度（更新于本轮开发，以下分析正文保留原样）**：#1（多线程隔离控制环）、#2（点云 raycast，原已实现）、#3（replan 文档一致）、#4（footprint 对角线安全）、#6（MPC 常量矩阵缓存）、#7（速度相关裕度，默认关）均已修复（见 `wip/tracker-yaw-and-review` + PR #1）；另新增脱困恢复 FSM。**L1 真因更正**：仿真运动 Z 漂移主因是 `small_point_lio` ESKF 加速度协方差饱和索引 bug（submodule `cf4d4df` 已修，~0.19m → ~0.02m），**非本报告"缺重力"假设**。L2（仿真雷达为快照、deskew 无关）、#5（多层占用）暂缓。
